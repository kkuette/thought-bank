...
| capacity deep / saturation (probe `capacity_deep`, jobs 113-115 en .hold) | à ~8192 writes (une vie cumulative, bins d'âge en octaves) : la page s'allume-t-elle (own−ablated) précisément là où le registre en superposition meurt (own−reset → 0, saturation longlife ~1024) ? | 🟡 staged 2026-07-14 : probe codée + smoke OK sur v3_reach (profil 109 répliqué aux petits W) ; checkpoints ALIGNÉS SUR FILL (formule user vérifiée sur cascade.py : 1re destruction à M + 2·M^depth writes — v3_reach d1=24, rehearsal d2=136, d3=1032, d4=8200 ; rétention totale M + 2·Σ M^k = 9368 @d4 = la spec) — d2/d3 comparés à capacité égale, pas à writes égaux ; jobs DÉPOSÉS 2026-07-14 (110/111 sortis) ; run local v3_reach fill 4 (2026-07-14) : registre intact à fill 4 (own−reset −0.4..−0.5 même sur fils détruits), own−foreign ≈ 0 partout, transient page web own−ablated −0.04..−0.06 (\|t\| 2-4) à fill 1-2 disparu à fill 4 ; 3 issues informatives (page s'allume = « jamais nécessaire » ; tout meurt = structurel ; registre tient = capacité sans page sous-estimée) | zero-shot, probe only (~4-6 h/ckpt d2, <1 h v3_reach) ; pool held (~8200 code / ~6000 web) plafonne d3 à fill ~8 ; contrôle own−foreign = spécificité d'adresse par bin |
...
| curriculum 2 phases à 97M (v350_curr_p1 local 3090 + v350_curr_p2 job 116) | le jumeau de COÛT du 350M (voie C, ÷3) : phase 1 batchée B=4 chunks fixes sans stack (recette v2b_mix, teacher ON, divmix, 1500 steps) puis phase 2 continued full-stack B=1 var_chunk (stack du 110, schedule v3_reach, 800 steps) — retombe-t-on sur les mécanismes du from-scratch joint ? | 🟡 lancé 2026-07-14 : phase 1 en local (farm/ root-only sur NFS ⇒ ckpt hors farm/, /mnt/tb/checkpoints/v350_curr_p1) ; VERT si carried ancres ±0.15 de 110 @2000 ET label-cue négatif aux probes ; job 116 en .hold, à déposer quand final.pt existe | comparaison à tokens non appariés (chunks fixes 512 vs var 128-512 ⇒ phase 1 voit ~1.6× tokens/step) — juger mécanismes + carried, pas la loss à step égal ; transitoire fixe→var_chunk toléré ~200 steps |
