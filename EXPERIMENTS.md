...
| v350 bring-up (Vast.ai, `configs/v350_bringup.yaml` + `vast/`) | the two missing sizing numbers: 350M steps/h off-WSL2 and true VRAM peak of the full stack at 768d (→ 4090 batch 1 vs A100 80GB B=4) | 🟡 kit ready (config + onstart script + CLI guide); local smoke validated config/model/data/lr_scale at ~17 GB forward, but backward crashes reproducibly under WSL2 (`device not ready`, 2×) = the 07-10 "386M spills" precedent with a precise signature | backward never run: first full step happens on the pod; measurement run only (40 steps), says nothing about bf16 training health (that is jobs 110/111) |
