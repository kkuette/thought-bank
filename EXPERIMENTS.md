...
| B1 trained taper (v2e_md256) (FINDINGS 2026-07-13 (5)) | does training close the 512→256 gap? | ✅ carried Δ −0.014 code / +0.042 web @800 (grid ±0.15) → trained taper 512→256 is FREE; 350M VRAM budgets with 512/256/128 stand | init v2b_md256_s4 vs v2c (equal post-init budget, seed noise uncontrolled); **md128 arm ✅ GREEN (job 112, FINDINGS 2026-07-14 (2))**: carried Δ +0.064/+0.044 @800 (grid ±0.15) ⇒ full taper 512→256→128 validated trained; reserve: md128 addressing is recency-fragile (junk-last label-cued +1.85/+1.10 vs ~0 @md512) + merge pricier (avg64 +0.50 code) ⇒ md512 live bank, 128 deep pages only |
...
| capacity deep / saturation (probe `capacity_deep`, jobs 113-115 en .hold) | à ~8192 writes (une vie cumulative, bins d'âge en octaves) : la page s'allume-t-elle (own−ablated) précisément là où le registre en superposition meurt (own−reset → 0, saturation longlife ~1024) ? | ✅ verdict 2026-07-14 (jobs 113/114/115, FINDINGS 2026-07-14 (3)) : branche (c) — le registre NE MEURT JAMAIS (own−reset −0.3..−1.5 à tous les fills, d1 jusqu'à 64×, d2 16×, 14 sources) ; la page contribue partout en milli-nats (own−ablated significatif : 10 lignes d1 → 76 d2 → 69 d3 ; arxiv d2 −0.05..−0.09 \|t\| jusqu'à 13.6 à tous âges, css/sql/js/html pareil ; dès W16, pas besoin de saturation) mais AUCUNE spécificité d'adresse à aucun fill (own−foreign ≈ 0 partout) ⇒ la saturation ne rendra pas la page adressée, la spécificité = entraînement de politique (OPTION 2) ; profondeur = multiplicateur page quasi gratuit | zero-shot, probe only (~4-6 h/ckpt d2, <1 h v3_reach) ; pool held (~8200 code / ~6000 web) plafonne d3 à fill ~8 ; contrôle own−foreign = spécificité d'adresse par bin |
| v350_rehearsal (job 110) | dress rehearsal: the FULL 350M recipe (divmix + D+G+G2 + cascade depth 2 map [0,0,0,0,1,2] + reach + bf16) trained from scratch at 97M — do the SFT-created mechanisms appear under joint training? | ✅ FINDINGS 2026-07-14 (2) : **the full stack emerges under joint from-scratch training** — addressing label-cue −1.08/−0.44, junk-last under label +0.07 (v2f signature), MID filter ≈ 0, GAP positive 14/14 sources @2000 ⇒ **no staged curriculum needed for the mechanisms**; page dead 4th strike (emergence −0.003/+0.004) ⇒ SFT-graft hypothesis eliminated; new signature: xdom bank = POISON (reset−xdom −0.96/−0.77 — the joint model trusts its read) | recency trap intact (blank junk-last +1.9) = GRPO ph.2's job; bank pollution = attack surface to guard in RL; bf16 held (no NaN) |
| v350_rehearsal_d3 (job 111) | controlled twin of 110, ONE variable: cascade_map [0,0,0,1,2,3] — depth 3 = total retention of an 8-file life (8+16+32 = 56 evictions covered) at the price of 3 live-bank layers instead of 4 | ✅ FINDINGS 2026-07-14 (2) : **depth 3 is FREE** — carried @2000 equal to d2 within 0.03-0.07 nat (d3 ahead on finemath/openstax/html); prediction carried d3 ≤ d2+0.3 ✓; page emergence still null (+0.002/+0.003) BUT capacity-curve page contribution −0.008/−0.022 web (\|t\|~6) = 10× d2, grows with depth — first double-milli page signal | 3 live layers suffice at 97M; mid-run GAP divergence @800 was reset-side noise (rule: carried, never GAP); page signal web-only, register-dominated — saturation scan (113-115) discriminates |
...
| curriculum 2 phases à 97M (v350_curr_p1 local 3090 + v350_curr_p2 job 116) | le jumeau de COÛT du 350M (voie C, ÷3) : phase 1 batchée B=4 chunks fixes sans stack (recette v2b_mix, teacher ON, divmix, 1500 steps) puis phase 2 continued full-stack B=1 var_chunk (stack du 110, schedule v3_reach, 800 steps) — retombe-t-on sur les mécanismes du from-scratch joint ? | 🟡 phase 1 FINIE 2026-07-14 (1500 steps, ~3h, zéro NaN) : GAP positif 14/14 @1500 (codeparrot +1.39 carried 5.877, web +0.94..+1.09, GAP-par-profondeur plat d2-d8) ; job 116 (phase 2 full-stack) déposé, pris par GPUrig0-gpu4 ; VERT si carried ancres ±0.15 de 110 @2000 ET label-cue négatif aux probes | comparaison à tokens non appariés (chunks fixes 512 vs var 128-512 ⇒ phase 1 voit ~1.6× tokens/step) — juger mécanismes + carried, pas la loss à step égal ; transitoire fixe→var_chunk toléré ~200 steps |
