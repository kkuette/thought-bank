...
| joint test (`multiturn_rule_joint.yaml`, 24+16 turns) | in progress — forces rehearsal in phase 1, then requires stopping it at the switch |

Bottom line so far: **memory policy — retention AND replacement — is
task-adaptive and emerges end-to-end**; no gate/LRU/allocation mechanism was
needed. Open costs: rehearsal precision (0.48 plateau → consolidation), read
generalization (recognition, not induction → code-space augmentation).
